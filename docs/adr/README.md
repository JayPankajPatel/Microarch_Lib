...
| [0018](0018-adder-integration-select-lfsr-decorrelation.md) | Adder integration test's select LFSR must be decorrelated from both operand encoders, not just the encoders from each other -- confirms ADR 0016/0017 generalize beyond the multiplier | Accepted |
| [0019](0019-minimum-width-for-seed-decorrelation.md) | Closes issue #3: no seed pair works below WIDTH=7 (hard structural threshold, not a seed-choice problem) -- WIDTH=4 integration tests are structural-verification-only | Accepted |
| [0020](0020-synchronous-reset-variants-for-fabric-targets.md) | Parallel `_sync`-suffixed RTL variants (sync-reset only) added alongside async-reset originals, for fabric FF BELs that have no async reset input | Accepted |
| [0021](0021-encoder-off-by-one-and-decoder-slot-race-fixes.md) | Fixes three independent-audit-found defects: encoder off-by-one (`<` vs `<=`), a decoder output-slot race (speculative ready decided before valid was externally visible), and encoder width-unsafe sizing (`1 << n` overflow at WIDTH>=32) | Accepted |
| [0022](0022-adder-select-bias-default-and-lint-hook-repair.md) | Adder's default `SELECT_LFSR_WIDTH` changed from 4 to 5 (was collision-prone with typical encoder defaults) and its inherent select bias documented; pre-commit lint hook repointed at the Bender-derived `pixi run lint` task with `GENUNNAMED`/`DECLFILENAME` noise suppressed so it's actually usable as a pass/fail gate | Accepted |
| [0023](0023-pio-seq-prim-isolation-test.md) | `pio_seq_prim` (from the sibling FABulous protocol-emulator project) imported as a new block, restructured to the two-process style, and isolation-tested to rule its own RTL out as the cause of a fabric-level "busy never asserts" bug -- confirmed innocent, so the fault is in fabric integration, not this primitive | Accepted |
